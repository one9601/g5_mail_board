...
<form action="/skin/board/inquiry/stat_sql.php" method="post" class="stat-edit-frm">
  <h3 class="tit">상태(관리자용)</h3>
  <input type="hidden" name="wr_id" value="<?php echo $view['wr_id']; ?>">
  <input type="hidden" name="bo_table" value="<?php echo $bo_table; ?>">

  <div class="stat-editbox">
    <select name="wr_10" id="wr_10" class="frm_input">
      <option value="제보 접수" <?php echo ($view['wr_10'] == '제보 접수') ? 'selected' : ''; ?>>제보 접수</option>
      <option value="담당자 확인중" <?php echo ($view['wr_10'] == '담당자 확인중') ? 'selected' : ''; ?>>담당자 확인중</option>
      <option value="답변완료" <?php echo ($view['wr_10'] == '답변완료') ? 'selected' : ''; ?>>답변완료</option>
    </select>
  </div>

  <button type="submit" class="stat-submitbtn">상태수정</button>
</form>
...
<?php } ?>
        ...
        <!-- 본문 내용 시작 { -->
        <div id="bo_v_con">
        <?php if($bo_table=='inquiry' || $bo_table =="report"){
		?>

		<style>
		table.table_css {width:100%;border-collapse: collapse;text-align: left;line-height: 1.5;}
		table.table_css thead th {padding: 10px;font-weight: bold;vertical-align: top;color: #369;border-bottom: 3px solid #036;}
		table.table_css tbody th {width: 15%;padding: 10px;font-weight: bold;vertical-align: top;border-bottom: 1px solid #ccc;background: #f3f6f7;}
		table.table_css td {width: 85%;padding: 10px;vertical-align: top;border-bottom: 1px solid #ccc;}
        #bo_v_img{margin-top: 40px;}
		</style>
